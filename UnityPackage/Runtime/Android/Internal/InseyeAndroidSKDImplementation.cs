...
namespace Inseye.Android.Internal
{
    ...
    internal sealed class InseyeAndroidSKDImplementation : ISDKImplementation, ISDKEventsBroker,
        ICalibrationCallback, ISDKStateManager,
        IInternalSDK
    {
...
#endif
            var calibrationProcedure = new AndroidCalibrationProcedure(this);
            try
            {
                var status = _javaLibrary.StartCalibrationProcedure(calibrationProcedure.CalibrationPointRequestPointer,
                    calibrationProcedure.CalibrationPointResponsePointer,
                    calibrationProcedure.CalibrationStatusPointer, calibrationProcedure.PointCounterPointer);
                switch (status)
                {
                    case ErrorCodes.Successful:
                        break;
                    case ErrorCodes.AnotherCalibrationIsOngoing:
                        throw new SDKCalibrationException("Another calibration is ongoing");
                    case ErrorCodes.UnknownErrorCheckErrorMessage:
                        throw new SDKCalibrationException(_javaLibrary.GetLastErrorMessage());
                    default:
                        throw new SDKCalibrationException(
                            $"Unexpected result of {nameof(ISDKImplementation.StartCalibration)}, error code {status:G}");
                }
        ...
        public bool IsCalibrated()
        {
            ...
            try
            {
                return _javaLibrary.IsEyeTrackerCalibrated();
            }
            finally
            {
                CheckState(); // validate state
        ...
        private void EnterEventSubscribedState()
        {
            ...
            try
            {
                errorCode = _javaLibrary.SubscribeToEvents();
            }
        ...
        void IInternalSDK.BeginRecordingRawData()
        {
            try
            {
                var code = _javaLibrary.BeginRecordingRawData();
                if (code == ErrorCodes.Successful)
                    return;
                if (code == ErrorCodes.UnknownErrorCheckErrorMessage)
                    throw new SDKInternalException(_javaLibrary.GetLastErrorMessage());
                throw new SDKInternalException($"Failed to start recording raw data, error code: {code:G}");
            }
            catch (AndroidJavaException androidException)
            {
                throw new SDKInternalException("Failed to start recording data due to AndroidJavaException.",
                    androidException);
            }
        }

        string IInternalSDK.EndRecordingRawData()
        {
            try
            {
                var path = _javaLibrary.EndRecordingRawData();
            ...
            {
                throw new SDKInternalException("Failed to end recording raw data due to AndroidJavaException",
                    androidJavaException);
            }
        }
